    ...
    def scrape_metadata(self, video_id) -> dict:
        ...
        while script_tag is None and retries <= 3:
            ...
            if script_tag is not None:
                ...
                retries += 1
                time.sleep(2.0)  # Longer sleep to avoid rate-limit
        else:
        ...
        # Check for TikTok error status codes
        status_code = video_detail.get("statusCode", 0)
        if status_code != 0:
            error_msg = video_detail.get("statusMsg", "Unknown error")
            raise ValueError(f"TikTok error {status_code}: {error_msg}")
        
        # Check if itemInfo exists
        if "itemInfo" not in video_detail or "itemStruct" not in video_detail.get("itemInfo", {}):
            raise KeyError("Video data not available in response")
        
        metadata = video_detail["itemInfo"]["itemStruct"]
